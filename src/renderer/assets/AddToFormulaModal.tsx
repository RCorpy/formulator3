...
export default function addToFormulaModal({
  show,
  onHide,
  product,
  setProduct,
  ...
  const handleSuccess = () => {
    const exactMatchRawArray = rawMatsArray.filter(
      (element) => element === currentSeachTerm
    );
    const exactFormulaArray = formulasArray.filter(
      (element) => element === currentSeachTerm
    );

    if (
      exactMatchRawArray.length + exactFormulaArray.length === 1 &&
      product.components[currentSeachTerm] == undefined &&
      searched !== currentSeachTerm
    ) {
      ...
      if (exactMatchRawArray.length + exactFormulaArray.length === 1) {
        console.log(' se han encontrado mas de 1 posibilidad ');
      }
      if (product.components[currentSeachTerm] == undefined) {
        console.log('ya existe');
      }
      if (searched !== currentSeachTerm) {
        console.log(
          'no se puede añadir un producto a su propia lista de componentes'
        );
      }
    }
  };
