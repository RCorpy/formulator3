...
export default function FabricacionNuevo({ fullData, setFullData }) {
  ...
  const getRecord = () => {
    window.electron.ipcRenderer.sendMessage('get-last-registro');
    window.electron.ipcRenderer.once('got-last-registro', (recievedData) => {
  ...
  const handleSuperposeRegistro = () => {
    window.electron.ipcRenderer.sendMessage('add-to-registro', {
      product: product,
      registro: registro,
      cantidad: cantidad,
    });
  ...
  const escribirRegistro = (product, cantidad, registro) => {
    window.electron.ipcRenderer.sendMessage('registrar', {
      product: product,
      registro: registro,
      cantidad: cantidad,
    });
  ...
  return (
    <>
      ...
      <InputGroup>
        ...
        <Form.Control onChange={() => handleRegistro(event)} value={registro} />
      </InputGroup>
      <InputGroup>
        <InputGroup.Text>Cantidad:</InputGroup.Text>
        <Form.Control onChange={() => handleCantidad(event)} value={cantidad} />
      </InputGroup>
